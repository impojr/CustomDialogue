...
public class ConversationManager : Singleton<ConversationManager>
{
    ...
    public void MoveInventoryLeft()
    {
        if (inventoryIndex == 0)
        {
            inventoryIndex = Inventory.Instance.items.Count - 1;
        } else
        {
            inventoryIndex--;
        }
    ...
    {
        if (inventoryIndex == Inventory.Instance.items.Count - 1)
        {
            inventoryIndex = 0;
        }
        else
    ...
    {
        inventoryItemImage.sprite = Inventory.Instance.items[inventoryIndex].image;
        inventoryItemName.text = Inventory.Instance.items[inventoryIndex].name;
    }
    ...
    {
        Item itemToPresent = Inventory.Instance.items[inventoryIndex];
